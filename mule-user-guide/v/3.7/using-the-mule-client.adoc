...
link:http://www.mulesoft.org/docs/site/current/apidocs/org/mule/module/client/MuleClient.html[MuleClient] is a simple interface for Java clients to send and receive messages from a Mule server and other applications. 
...
. The Mule URL endpoint: any valid link:/mule-user-guide/v/3.7/mule-endpoint-uris[Mule Endpoint] used to determine the transport, endpoint, and other information about delivery of the message. This can also be an endpoint name stored in configuration.
...
If you are using the Mule client in the same classloader (e.g., a Web App or Mule stand-alone), the client has access to the server configuration. For example, if you had some endpoints defined in your server configuration file:
...
Essentially, the Mule client has the same configuration information as the Mule server, since they will both have access to the same registry.
...
For SOAP support, you can use the Mule link:/mule-user-guide/v/3.7/cxf-module-reference[CXF Module Reference].
...
This code attempts to receive a message from a mailbox called `ross` on `mail.my.org` and will return after five seconds if no message was received. Calling `request()` works for all Mule supported transports, but it is more usual to make event request calls where there is a store to be queried such as a queue, file directory, or some other repository.
...
To gain access to the `MuleEventContext` inside your flows, you can implement the link:http://www.mulesoft.org/docs/site/current/apidocs/org/mule/api/lifecycle/Callable.html[org.mule.api.lifecycle.Callable] interface.
...
To handle situations where multiple results occur, Mule has introduced the  link:http://www.mulesoft.org/docs/site/current/apidocs/org/mule/api/MuleMessageCollection.html[org.mule.api.MuleMessageCollection] message type.

This type of message contains all message results in the order they were received. Note that link:http://www.mulesoft.org/docs/site/current/apidocs/org/mule/api/MuleMessageCollection.html[org.mule.api.MuleMessageCollection] extends http://www.mulesoft.org/docs/site/current/apidocs/org/mule/api/MuleMessage.html[org.mule.api.MuleMessage], so the interface is similar. If there are multiple results, the `MuleMessage.getPayload()` method returns a `java.util.List` containing the payloads of each of the returned messages.
...
The Mule client allows you to make synchronous calls without blocking by using the `sendAsync()`method, which returns a link:http://www.mulesoft.org/docs/site/current/apidocs/org/mule/api/FutureMessageResult.html[FutureMessageResult] that can be queried later.
...
If you do not set the wire format, the serialization format is used. For more information on transformers, see link:/mule-user-guide/v/3.7/using-transformers[Using Transformers].
...
WARNING: code does not compile. Not sure when sendDirect last worked, but it certainly doesn't work now
